...
public class GameServiceImpl implements GameService {
	...
	public Game beginGame(String user_id1, String user_id2) {
		...
		if (person1 != null)
			person1.setHead_url(Const.IP + person1.getHead_url());
		Person person2 = getListRandomPerson(person_id_list2);
		if (person2 != null)
			person2.setHead_url(Const.IP + person2.getHead_url());
		Person person3 = getListRandomPerson(person_id_list3);
		if (person3 != null)
			person3.setHead_url(Const.IP + person3.getHead_url());
		Person person4 = getListRandomPerson(person_id_list4);
		if (person4 != null)
			person4.setHead_url(Const.IP + person4.getHead_url());
		Person person5 = getListRandomPerson(person_id_list5);
		if (person5 != null)
			person5.setHead_url(Const.IP + person5.getHead_url());
	...
	public void doGame(GameRequest request, Game game) {
		...
		if (code == 0) {
			...
			if (prePerson != null) {
				my_response.setPerson_id(prePerson.getPerson_id());
				// 开始pk
				Person myPerson = personDao.getPersonById(person_id);
				System.out.println(
						"开始pk，两个武将分别是：prePerson" + prePerson.getPerson_id() + "  myPerson" + myPerson.getPerson_id());
				System.out.println("武力值为:" + "pre=" + prePerson.getAbility() + "my=" + myPerson.getAbility());
				Integer isWin = 0; // 0,1,2 分别代表平局，对方赢，我赢
				// 判断相克关系
				if ((myPerson.getPerson_field() == (prePerson.getPerson_field() + 1))
						|| (prePerson.getPerson_field() == 5 && myPerson.getPerson_field() == 1)) {
					// 那我输了
					isWin = 1;
					System.out.println("相克");
				} else if ((prePerson.getPerson_field() == (myPerson.getPerson_field() + 1))
						|| (prePerson.getPerson_field() == 1 && myPerson.getPerson_field() == 5)) {
					//我赢了
					isWin = 2;
					System.out.println("相克");
				} else {
				...
				webSocketHandler.sendToUserById(request.getOpposite_id(), JSON.toJSONString(opposite_response));
				webSocketHandler.sendToUserById(request.getMy_id(), JSON.toJSONString(my_response));
				// 最后清空preperson
